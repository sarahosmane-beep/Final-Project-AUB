...
function render() {
  ...
  const visible = tasks.filter(task => `${task.title} ${task.description || ""} ${task.assignee || ""}`.toLowerCase().includes(search));
  ...
  el("taskList").innerHTML = visible.map(task => `
    <article class="task" data-id="${task.id}"><div>
      ...
      <div class="meta"><span class="pill ${task.status}">${label(task.status)}</span><span class="pill ${task.priority}">${task.priority}</span>${task.assignee ? `<span class="assignee">Assigned to ${escapeHtml(task.assignee)}</span>` : ""}</div>
...
["statusFilter", "priorityFilter"].forEach(id => el(id).addEventListener("change", () => loadTasks().catch(showError)));
...
el("taskList").addEventListener("click", async event => {
  ...
  if (button.dataset.action === "delete") {
    ...
    el("status").value = task.status; el("priority").value = task.priority; el("assignee").value = task.assignee || "";
